...
#include "ArenaSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EArenaAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
UENUM(BlueprintType)
enum class EArenaGamepadSensitivity : uint8
{
...
/**
 * UArenaSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UArenaSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UArenaSettingsShared, FOnSettingChangedEvent, UArenaSettingsShared* Settings);
	FOnSettingChangedEvent OnSettingChanged;

public:

	UArenaSettingsShared();
	...
	static UArenaSettingsShared* CreateTemporarySettings(const UArenaLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UArenaSettingsShared* LoadOrCreateSettings(const UArenaLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UArenaSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UArenaLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EArenaAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EArenaAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EArenaAllowBackgroundAudioSetting AllowAudioInBackground = EArenaAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EArenaGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EArenaGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EArenaGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EArenaGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EArenaGamepadSensitivity GamepadLookSensitivityPreset = EArenaGamepadSensitivity::Normal;
	UPROPERTY()
	EArenaGamepadSensitivity GamepadTargetingSensitivityPreset = EArenaGamepadSensitivity::Normal;
