...
#include "ArenaSettingsLocal.generated.h"
...
USTRUCT()
struct FArenaScalabilitySnapshot
{
	GENERATED_BODY()

	FArenaScalabilitySnapshot();
 ...
 */
UCLASS()
class UArenaSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	static UArenaSettingsLocal* Get();
...
public:
	...
	EArenaStatDisplayMode GetPerfStatDisplayState(EArenaDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EArenaDisplayablePerformanceStat Stat, EArenaStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UArenaSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EArenaDisplayablePerformanceStat, EArenaStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FArenaScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FArenaScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UArenaSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
