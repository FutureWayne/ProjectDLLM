...
#include "ArenaPerformanceSettings.generated.h"
...
USTRUCT()
struct FArenaQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FArenaPerformanceStatGroup
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere)
	TSet<EArenaDisplayablePerformanceStat> AllowedStats;
};

// How hare frame pacing and overall graphics settings controlled/exposed for the platform?
UENUM()
enum class EArenaFramePacingMode : uint8
{
...
class UArenaPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UArenaPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UArenaPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FArenaQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	EArenaFramePacingMode FramePacingMode = EArenaFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by Arena.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==EArenaFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="Arena Performance Settings"))
class UArenaPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UArenaPerformanceSettings();
...
public:
	...
	TArray<FArenaPerformanceStatGroup> UserFacingPerformanceStats;
};
