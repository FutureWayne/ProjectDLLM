...
#include "ArenaAudioSettings.generated.h"
...
USTRUCT()
struct ARENA_API FArenaSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "ArenaAudioSettings"))
class ARENA_API UArenaAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FArenaSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FArenaSubmixEffectChainMap> LDRAudioSubmixEffectChain;
