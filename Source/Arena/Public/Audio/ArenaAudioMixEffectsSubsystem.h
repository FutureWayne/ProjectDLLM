...
#include "ArenaAudioMixEffectsSubsystem.generated.h"
...
USTRUCT()
struct FArenaAudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 * Arena Audio Settings.
 */
UCLASS()
class ARENA_API UArenaAudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	// Default Sound Control Bus Mix retrieved from the Arena Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> DefaultBaseMix = nullptr;

	// Loading Screen Sound Control Bus Mix retrieved from the Arena Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> LoadingScreenMix = nullptr;

	// User Sound Control Bus Mix retrieved from the Arena Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> UserMix = nullptr;

	// Overall Sound Control Bus retrieved from the Arena Audio Settings and linked to the UI and game settings in ArenaSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> OverallControlBus = nullptr;

	// Music Sound Control Bus retrieved from the Arena Audio Settings and linked to the UI and game settings in ArenaSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> MusicControlBus = nullptr;

	// SoundFX Sound Control Bus retrieved from the Arena Audio Settings and linked to the UI and game settings in ArenaSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> SoundFXControlBus = nullptr;

	// Dialogue Sound Control Bus retrieved from the Arena Audio Settings and linked to the UI and game settings in ArenaSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> DialogueControlBus = nullptr;

	// VoiceChat Sound Control Bus retrieved from the Arena Audio Settings and linked to the UI and game settings in ArenaSettingsLocal
	UPROPERTY(Transient)
	...
	UPROPERTY(Transient)
	TArray<FArenaAudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FArenaAudioSubmixEffectsChain> LDRSubmixEffectChain;
