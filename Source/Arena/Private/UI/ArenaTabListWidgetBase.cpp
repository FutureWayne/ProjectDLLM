...
void UArenaTabListWidgetBase::NativeOnInitialized()
{
	Super::NativeOnInitialized();
}

void UArenaTabListWidgetBase::NativeConstruct()
{
...
void UArenaTabListWidgetBase::NativeDestruct()
{
	for (FArenaTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UArenaTabListWidgetBase::GetPreregisteredTabInfo(const FName TabNameId, FArenaTabDescriptor& OutTabInfo)
{
	const FArenaTabDescriptor* const FoundTabInfo = PreregisteredTabInfoArray.FindByPredicate([&](FArenaTabDescriptor& TabInfo) -> bool
	{
...
void UArenaTabListWidgetBase::SetTabHiddenState(FName TabNameId, bool bHidden)
{
	for (FArenaTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
bool UArenaTabListWidgetBase::RegisterDynamicTab(const FArenaTabDescriptor& TabDescriptor)
{
...
void UArenaTabListWidgetBase::HandlePreLinkedSwitcherChanged()
{
	for (const FArenaTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
...
void UArenaTabListWidgetBase::HandlePostLinkedSwitcherChanged()
{
...
void UArenaTabListWidgetBase::HandleTabCreation_Implementation(FName TabId, UCommonButtonBase* TabButton)
{
	FArenaTabDescriptor* TabInfoPtr = nullptr;
	
	FArenaTabDescriptor TabInfo;
	...
	if (TabButton->GetClass()->ImplementsInterface(UArenaTabButtonInterface::StaticClass()))
	{
		...
		{
			IArenaTabButtonInterface::Execute_SetTabLabelInfo(TabButton, *TabInfoPtr);
		}
	}
...
bool UArenaTabListWidgetBase::IsFirstTabActive() const
{
...
bool UArenaTabListWidgetBase::IsLastTabActive() const
{
	if (PreregisteredTabInfoArray.Num() > 0)
	{
...
bool UArenaTabListWidgetBase::IsTabVisible(FName TabId)
{
...
int32 UArenaTabListWidgetBase::GetVisibleTabCount()
{
	int32 Result = 0;
...
void UArenaTabListWidgetBase::SetupTabs()
{
	for (FArenaTabDescriptor& TabInfo : PreregisteredTabInfoArray)
	{
		if (TabInfo.bHidden)
