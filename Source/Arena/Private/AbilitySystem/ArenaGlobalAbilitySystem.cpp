...
void FGlobalAppliedAbilityList::AddToASC(const TSubclassOf<UGameplayAbility>& Ability, UArenaAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedAbilityList::RemoveFromASC(UArenaAbilitySystemComponent* ASC)
{
	if (FGameplayAbilitySpecHandle* SpecHandle = Handles.Find(ASC))
	{
...
void FGlobalAppliedEffectList::AddToASC(TSubclassOf<UGameplayEffect> Effect, UArenaAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedEffectList::RemoveFromASC(UArenaAbilitySystemComponent* ASC)
{
	if (FActiveGameplayEffectHandle* EffectHandle = Handles.Find(ASC))
	{
...
UArenaGlobalAbilitySystem::UArenaGlobalAbilitySystem()
{
	
}

void UArenaGlobalAbilitySystem::ApplyAbilityToAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && (!AppliedAbilities.Contains(Ability)))
	{
		FGlobalAppliedAbilityList& Entry = AppliedAbilities.Add(Ability);		
		for (UArenaAbilitySystemComponent* ASC : RegisteredASCs)
		{
...
void UArenaGlobalAbilitySystem::ApplyEffectToAll(TSubclassOf<UGameplayEffect> Effect)
{
...
void UArenaGlobalAbilitySystem::RemoveAbilityFromAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && AppliedAbilities.Contains(Ability))
	{
...
void UArenaGlobalAbilitySystem::RemoveEffectFromAll(TSubclassOf<UGameplayEffect> Effect)
{
	if ((Effect.Get() != nullptr) && AppliedEffects.Contains(Effect))
	{
...
void UArenaGlobalAbilitySystem::RegisterASC(UArenaAbilitySystemComponent* ASC)
{
	check(ASC);
...
void UArenaGlobalAbilitySystem::UnregisterASC(UArenaAbilitySystemComponent* ASC)
{
	check(ASC);
	for (auto& Entry : AppliedAbilities)
	{
