...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ArenaSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Arena.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Arena.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Arena.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Arena.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Arena.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Arena.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Arena.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Arena.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Arena.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Arena.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FArenaScalabilitySnapshot::FArenaScalabilitySnapshot()
{
...
namespace ArenaSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FArenaScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
		...
		Mode = FArenaScalabilitySnapshot();
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UArenaSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UArenaSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UArenaSettingsLocal::UArenaSettingsLocal()
{
...
void UArenaSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UArenaSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UArenaPlatformSpecificRenderingSettings* PlatformSettings = UArenaPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EArenaFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	ArenaSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UArenaSettingsLocal::ResetToCurrentSettings()
{
...
void UArenaSettingsLocal::BeginDestroy()
{
...
UArenaSettingsLocal* UArenaSettingsLocal::Get()
{
	return GEngine ? CastChecked<UArenaSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UArenaSettingsLocal::ConfirmVideoMode()
{
...
float UArenaSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UArenaPlatformSpecificRenderingSettings* PlatformSettings = UArenaPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EArenaFramePacingMode::DesktopStyle)
	{
...
int32 UArenaSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return ArenaSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UArenaSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FArenaScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UArenaSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UArenaSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UArenaSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UArenaSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UArenaSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UArenaSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EArenaStatDisplayMode UArenaSettingsLocal::GetPerfStatDisplayState(EArenaDisplayablePerformanceStat Stat) const
{
	if (const EArenaStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EArenaStatDisplayMode::Hidden;
	}
}

void UArenaSettingsLocal::SetPerfStatDisplayState(EArenaDisplayablePerformanceStat Stat, EArenaStatDisplayMode DisplayMode)
{
	if (DisplayMode == EArenaStatDisplayMode::Hidden)
	{
...
float UArenaSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UArenaSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UArenaSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UArenaSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UArenaSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UArenaSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UArenaSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UArenaSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UArenaSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UArenaSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UArenaSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UArenaSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UArenaSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UArenaSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UArenaSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UArenaSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return ArenaSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UArenaSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return ArenaSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UArenaSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UArenaSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UArenaPlatformSpecificRenderingSettings* PlatformSettings = UArenaPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EArenaFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return ArenaSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UArenaSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UArenaPlatformSpecificRenderingSettings* PlatformSettings = UArenaPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EArenaFramePacingMode::MobileStyle)
	{
...
void UArenaSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UArenaSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UArenaPlatformSpecificRenderingSettings* PlatformSettings = UArenaPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EArenaFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = ArenaSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UArenaSettingsLocal::ClampMobileQuality()
{
	const UArenaPlatformSpecificRenderingSettings* PlatformSettings = UArenaPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EArenaFramePacingMode::MobileStyle)
	{
...
void UArenaSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = ArenaSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UArenaSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = ArenaSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = ArenaSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UArenaSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UArenaSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UArenaSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UArenaSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UArenaSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = ArenaSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UArenaSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UArenaSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UArenaAudioMixEffectsSubsystem* ArenaAudioMixEffectsSubsystem = World->GetSubsystem<UArenaAudioMixEffectsSubsystem>())
			{
				ArenaAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UArenaSettingsLocal::CanRunAutoBenchmark() const
{
	const UArenaPlatformSpecificRenderingSettings* PlatformSettings = UArenaPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UArenaSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UArenaSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UArenaSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UArenaSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UArenaSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UArenaSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UArenaSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UArenaSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UArenaSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UArenaSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UArenaSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UArenaSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UArenaSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UArenaSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UArenaSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UArenaSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UArenaSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EArenaFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = ArenaSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UArenaSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UArenaSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UArenaSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UArenaAudioSettings* ArenaAudioSettings = GetDefault<UArenaAudioSettings>())
			{
				...
				if (UObject* ObjPath = ArenaAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Arena Audio Settings."));
					}
				}
			}
		}
...
void UArenaSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UArenaSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UArenaPlatformSpecificRenderingSettings* PlatformSettings = UArenaPlatformSpecificRenderingSettings::Get();
	const TArray<FArenaQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FArenaQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	{
	case EArenaFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EArenaFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EArenaFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UArenaSettingsLocal::UpdateConsoleFramePacing()
{
...
void UArenaSettingsLocal::UpdateDesktopFramePacing()
{
...
void UArenaSettingsLocal::UpdateMobileFramePacing()
{
...
void UArenaSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
