...
#include "Settings/CustomSettings/ArenaSettingValueDiscrete_Resolution.h"
...
{
public:
	FGameSettingEditCondition_FramePacingMode(EArenaFramePacingMode InDesiredMode, EFramePacingEditCondition InMatchMode = EFramePacingEditCondition::EnableIf)
	...
	{
		const EArenaFramePacingMode ActualMode = UArenaPlatformSpecificRenderingSettings::Get()->FramePacingMode;
...
private:
	EArenaFramePacingMode DesiredMode;
...
public:
	...
	virtual void GatherEditState(const ULocalPlayer* InLocalPlayer, FGameSettingEditableState& InOutEditState) const override
	{
		if (!UArenaPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings)
		{
	...
	virtual void SettingChanged(const ULocalPlayer* LocalPlayer, UGameSetting* Setting, EGameSettingChangeReason Reason) const override
	{
		...
		const UArenaLocalPlayer* ArenaLocalPlayer = CastChecked<UArenaLocalPlayer>(LocalPlayer);
		ArenaLocalPlayer->GetLocalSettings()->ApplyScalabilitySettings();
	}
...
UGameSettingCollection* UArenaGameSettingRegistry::InitializeVideoSettings(UArenaLocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			UArenaSettingValueDiscrete_Resolution* Setting = NewObject<UArenaSettingValueDiscrete_Resolution>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UArenaSettingsShared>()->GetColorBlindMode());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UArenaSettingsShared>()->GetColorBlindStrength());
		...
		//----------------------------------------------------------------------------------
		{
			UArenaSettingAction_SafeZoneEditor* Setting = NewObject<UArenaSettingAction_SafeZoneEditor>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			for (const FArenaQualityDeviceProfileVariant& Variant : PlatformSettings->UserFacingDeviceProfileOptions)
			{
		...
		//----------------------------------------------------------------------------------
		{
			...
			UArenaSettingValueDiscrete_MobileFPSType* Setting = NewObject<UArenaSettingValueDiscrete_MobileFPSType>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EArenaFramePacingMode::MobileStyle));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetCustomAction([](ULocalPlayer* LocalPlayer)
			{
				...
				{
					UArenaSettingsLocal::Get()->ResetToMobileDeviceDefaults();
				}
				else
				{
					const UArenaLocalPlayer* ArenaLocalPlayer = CastChecked<UArenaLocalPlayer>(LocalPlayer);
					...
					ArenaLocalPlayer->GetLocalSettings()->RunAutoBenchmark(bImmediatelySaveState);
				}
			});
			...
			{
				const UArenaPlatformSpecificRenderingSettings* PlatformSettings = UArenaPlatformSpecificRenderingSettings::Get();
				const bool bCanUseDueToMobile = (PlatformSettings->FramePacingMode == EArenaFramePacingMode::MobileStyle);

				const UArenaLocalPlayer* ArenaLocalPlayer = CastChecked<UArenaLocalPlayer>(LocalPlayer);
				const bool bCanBenchmark = ArenaLocalPlayer->GetLocalSettings()->CanRunAutoBenchmark();
		...
		//----------------------------------------------------------------------------------
		{
			UArenaSettingValueDiscrete_OverallQuality* Setting = NewObject<UArenaSettingValueDiscrete_OverallQuality>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EArenaFramePacingMode::ConsoleStyle, EFramePacingEditCondition::DisableIf));
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EArenaFramePacingMode::DesktopStyle));
...
void AddFrameRateOptions(UGameSettingValueDiscreteDynamic_Number* Setting)
{
	...
	for (int32 Rate : GetDefault<UArenaPerformanceSettings>()->DesktopFrameRateLimits)
	{
...
void UArenaGameSettingRegistry::InitializeVideoSettings_FrameRates(UGameSettingCollection* Screen, UArenaLocalPlayer* InLocalPlayer)
{
	//----------------------------------------------------------------------------------
	{
		UGameSettingValueDiscreteDynamic_Number* Setting = NewObject<UGameSettingValueDiscreteDynamic_Number>();
		...
		Setting->SetDefaultValue(GetDefault<UArenaSettingsLocal>()->GetFrameRateLimit_OnBattery());
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UArenaSettingsLocal>()->GetFrameRateLimit_InMenu());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EArenaFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UArenaSettingsLocal>()->GetFrameRateLimit_WhenBackgrounded());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EArenaFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UArenaSettingsLocal>()->GetFrameRateLimit_Always());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EArenaFramePacingMode::DesktopStyle));
