...
UGameSettingCollection* UArenaGameSettingRegistry::InitializeGameplaySettings(UArenaLocalPlayer* InLocalPlayer)
{
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			UArenaSettingValueDiscrete_Language* Setting = NewObject<UArenaSettingValueDiscrete_Language>();
...

	{
		...
		// 	Setting->SetDefaultValue(GetDefault<UArenaSettingsLocal>()->ShouldAutoRecordReplays());
		//
		// 	Setting->AddEditCondition(FWhenPlayingAsPrimaryPlayer::Get());
		// 	Setting->AddEditCondition(FWhenPlatformHasTrait::KillIfMissing(UArenaReplaySubsystem::GetPlatformSupportTraitTag(), TEXT("Platform does not support saving replays")));
		//
		...
		// 	Setting->SetDefaultValue(GetDefault<UArenaSettingsLocal>()->GetNumberOfReplaysToKeep());
