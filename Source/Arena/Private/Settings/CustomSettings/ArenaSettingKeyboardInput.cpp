...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ArenaSettingKeyboardInput)

class ULocalPlayer;

#define LOCTEXT_NAMESPACE "ArenaSettings"

namespace Arena::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("ArenaErrors_UnknownMappingName", "Unknown Mapping");
}


UArenaSettingKeyboardInput::UArenaSettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText UArenaSettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return Arena::ErrorMessages::UnknownMappingName;
}

FText UArenaSettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* UArenaSettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* UArenaSettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* UArenaSettingKeyboardInput::GetUserSettings() const
{
	if(UArenaLocalPlayer* ArenaLocalPlayer = Cast<UArenaLocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = ArenaLocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void UArenaSettingKeyboardInput::OnInitialized()
{
...
void UArenaSettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText UArenaSettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
FText UArenaSettingKeyboardInput::GetPrimaryKeyText() const
{
	return GetKeyTextFromSlot(EPlayerMappableKeySlot::First);
}

FText UArenaSettingKeyboardInput::GetSecondaryKeyText() const
{
	return GetKeyTextFromSlot(EPlayerMappableKeySlot::Second);
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void UArenaSettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void UArenaSettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void UArenaSettingKeyboardInput::RestoreToInitial()
{
...
bool UArenaSettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void UArenaSettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool UArenaSettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
