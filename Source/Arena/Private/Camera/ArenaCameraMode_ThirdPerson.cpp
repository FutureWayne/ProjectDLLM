...
#include "Camera/ArenaCameraMode_ThirdPerson.h"
#include "Camera/ArenaCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/ArenaPenetrationAvoidanceFeeler.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ArenaCameraMode_ThirdPerson)

namespace ArenaCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UArenaCameraMode_ThirdPerson::UArenaCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FArenaPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FArenaPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FArenaPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FArenaPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FArenaPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FArenaPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FArenaPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UArenaCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UArenaCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UArenaCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IArenaCameraAssistInterface* TargetControllerAssist = Cast<IArenaCameraAssistInterface>(TargetController);

	IArenaCameraAssistInterface* TargetActorAssist = Cast<IArenaCameraAssistInterface>(TargetActor);
	...
	IArenaCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IArenaCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IArenaCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IArenaCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UArenaCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IArenaCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FArenaPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(ArenaCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UArenaCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UArenaCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
